...
/**
  ...
  */
class UserCacheRepository @Inject()(cache: CacheProxy) extends UserRepository with LazyLogging {
  ...
  override def create(newUser: AppUser): Future[AppUser] = {
    ...
    findAll().flatMap {
      ...
      case l => cache.set(prefix, user +: l).map { _ => user }
    }
  }

  override def find(userId: Int): Future[Option[AppUser]] =
    cache.get[AppUser](prefix).map(_.flatMap(_.find(_.id == userId)))


  def findAll(): Future[Seq[AppUser]] =
    cache.get[AppUser](prefix).map(_.getOrElse(Nil))

  override def find(name: String): Future[Option[AppUser]] = {
      cache.get[AppUser](prefix).map(
        _.flatMap(
          _.find(
